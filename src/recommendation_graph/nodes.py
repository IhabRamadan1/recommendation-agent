...
Technical: nodes adapt GraphState; ranking logic lives in ranking.compute_ranked_items.
"""
        ...
        try:
            ...
            if item.id in seen_ids:
                return {
                    ...
                    "catalog_ok": False,
                    "errors": [
                        f"Duplicate catalog id {item.id!r} at index {idx}; "
                        "reference data is malformed."
                    ],
                }
...
def rank_items(state: dict[str, Any]) -> dict[str, Any]:
    """LangGraph adapter: GraphState → compute_ranked_items → GraphState patch."""
    ...
    try:
        ...
        top = compute_ranked_items(profile, catalog, limit=5)
    except Exception as exc:  # noqa: BLE001
        return {
            "ranked_items": [],
            "rank_failed": True,
...
def write_narrative(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # Prefer ranked_items if present; otherwise use the shared ranking helper.
    if not ranked_raw and state.get("catalog"):
        try:
            profile = UserProfile.model_validate(state.get("profile") or {})
            catalog = [CatalogItem.model_validate(c) for c in state.get("catalog") or []]
            ranked = compute_ranked_items(profile, catalog, limit=5)
            ranked_raw = [r.model_dump() for r in ranked]
        except Exception as exc:  # noqa: BLE001
            return {
                "narrative": "",
                "narrative_failed": True,
                "errors": [f"Narrative skipped: could not obtain ranked items: {exc}"],
            }
        if not ranked_raw:
            return {
                "narrative": "",
...
def validate_output(state: dict[str, Any]) -> dict[str, Any]:
    ...
    passed, messages = validate_recommendation_state(
        ...
        narrative_failed=bool(state.get("narrative_failed")),
